...
export default function GestaoPage() {
  ...
  // Agrupa todas as atividades por funcionário (ativas + historico)
  const todos = [...ativas, ...historico];
  const porFuncionario = {};
  todos.forEach((a) => {
    if (!porFuncionario[a.funcionario]) {
      porFuncionario[a.funcionario] = { nome: a.funcionario, atividades: [] };
    }
    porFuncionario[a.funcionario].atividades.push(a);
  });
  // Ordena: funcionários com atividade ativa primeiro
  const funcionarios = Object.values(porFuncionario).sort((a, b) => {
    const aAtiva = a.atividades.some((x) => x.status === "ativo") ? 0 : 1;
    const bAtiva = b.atividades.some((x) => x.status === "ativo") ? 0 : 1;
      ...
      <main className="main">
        <div className="tabs">
          <button className={aba === "andamento" ? "tab active" : "tab"} onClick={() => setAba("andamento")}>
            Funcionários
            {ativas.length > 0 && <span className="badge">{ativas.length} ativo{ativas.length > 1 ? "s" : ""}</span>}
          </button>
                  ...
                  return (
                    <div className={`func-card ${atividadeAtiva ? "func-card--ativa" : ""}`} key={f.nome}>
                      ...
                      <div className="func-header">
                        <div className="avatar" style={{ background: getColor(f.nome) }}>
                          {getInitials(f.nome)}
                        </div>
                      ...
                      {finalizadas.length > 0 && (
                        <div className="ativ-lista">
                          ...
                          {finalizadas.length > 5 && (
                            <span className="ativ-mais">+{finalizadas.length - 5} mais</span>
                          )}
                        </div>
                      )}
                    </div>
            ...
            {historico.length === 0 ? (
              ...
              <>
                <div className="resumo-grid">
                  ...
                  <div className="resumo-card">
                    <span className="resumo-num">{rankingFunc.length}</span>
                    <span className="resumo-label">Funcionários ativos</span>
                  </div>
                  <div className="resumo-card">
                    <span className="resumo-num">
                      {(() => { const total = historico.reduce((acc, a) => acc + calcMinutes(a), 0); const h = Math.floor(total / 60); const m = total % 60; return h > 0 ? `${h}h ${m}m` : `${m}m`; })()}
                    </span>
                    <span className="resumo-label">Tempo total registrado</span>
                  </div>
                  <div className="resumo-card">
                    <span className="resumo-num">{rankingAtiv.length}</span>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 600px) {
          .header-stats { gap: 1rem; } .stat-num { font-size: 18px; }
          .tabs { width: 100%; } .tab { flex: 1; justify-content: center; padding: 8px 6px; font-size: 12px; }
        }
      `}</style>
    </div>
  );
